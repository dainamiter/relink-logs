...
use gbfr_logs::{
    db, legality, overmastery, parser, portable, settings_db, synthesis, transmarvel, window_state,
};
...
/// Config file the injected hook reads ONCE at startup. It sits in the portable
/// tree's `config/` folder, next to the hook's own log — the hook resolves the
/// same directory from `GBFR_LOGS_DATA_DIR` (set in `portable::prepare`) or,
/// failing that, from its own module path, so neither half ever guesses at an
/// AppData path.
fn hook_config_path() -> Result<std::path::PathBuf, String> {
    let path = portable::config_dir();
    std::fs::create_dir_all(path).map_err(|e| e.to_string())?;
    Ok(portable::hook_config_file())
}
...
/// the app's log level/target: `config/reload-debug.log` in the portable tree,
/// next to the hook's own gbfr-logs.txt. Also mirrors to the normal `info!`
/// sink. The file write happens only in debug builds; release still gets the
/// `info!`.
#[cfg(windows)]
fn reload_dbg(msg: &str) {
    ...
    #[cfg(debug_assertions)]
    {
        ...
        let path = portable::config_dir().join("reload-debug.log");
...
fn reset_meter_window(app_handle: AppHandle) {
    ...
    // The stored geometry has to go too, or the next launch restores exactly
    // what was just reset away. This drops the logs window's entry as well,
    // which is what the tray's "Reset Windows" does — one notion of "reset",
    ...
    match event {
        ...
        SystemTrayEvent::MenuItemClick { id, .. } => match id.as_str() {
            ...
            "reset_windows" => {
                ...
                // The stored geometry has to go too, or the next launch
                // restores exactly what was just reset away.
                window_state::clear();
            }
            "quit" => {
                window_state::save(handle);
                handle.exit(0)
            }
            #[cfg(all(windows, debug_assertions))]
            "reload_hook" => {
...
fn main() {
    // First, before anything reads a path or starts a webview: move every
    // writable file into the portable tree beside the executable and redirect
    // the AppData lookups the dependencies still perform. Windows resolves
    // those from the registry, so no launcher script could have done this.
    portable::prepare();
    ...
    tauri::Builder::default()
        ...
        .on_window_event(|event| match event.event() {
            ...
            // Windows hide instead of closing, so a geometry write on close
            // would never fire; these are the moves and resizes that matter.
            // `save` is a no-op unless something actually changed.
            tauri::WindowEvent::Moved(_) | tauri::WindowEvent::Resized(_) => {
                window_state::save(&event.window().app_handle());
            }
            _ => {}
        })
        ...
        .setup(|app| {
            ...
            // Both windows exist by now (they come from tauri.conf.json), so
            // the geometry saved last session can be applied before the user
