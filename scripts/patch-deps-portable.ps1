...
# tauri ships `scripts/` for its include_str! templates; wry is src-only.
Invoke-VendoredPatch -Name 'tauri' -Version '1.8.3' `
    -RelativeFile 'src/manager.rs' -Before $tauriBefore -After $tauriAfter `
    -RequiredDirs @('scripts', 'src') | Out-Null

Invoke-VendoredPatch -Name 'wry' -Version '0.24.12' `
    -RelativeFile 'src/webview/webview2/mod.rs' -Before $wryBefore -After $wryAfter `
    -RequiredDirs @('src') | Out-Null
